...
// Model untuk tabel kitab mahasiswi dan dewasa putri
class KitabMahasiswi {
  // Method untuk mendapatkan semua data
  static async getAll() {
    try {
      const [rows] = await db.query(`
        SELECT km.*, sk.nama_kitab, sk.penerbit 
        FROM kitab_mahasiswi_dewasa km
  ...
  static async create(data) {
    ...
    try {
      ...
      const [result] = await connection.query(
        `INSERT INTO kitab_mahasiswi_dewasa 
  ...
  static async update(id, data) {
    ...
    try {
      ...
      const [oldData] = await connection.query(
        'SELECT kitab_id, jumlah FROM kitab_mahasiswi_dewasa WHERE id = ?',
        [id]
      );
  ...
  static async delete(id) {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        // Hapus data
        const [result] = await connection.query(
          'DELETE FROM kitab_mahasiswi_dewasa WHERE id = ?',
          [id]
        );
